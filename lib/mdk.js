    ...
    config.get('mdk_analytics_enabled',function(err, res) {
        ...
        if (res === undefined)
        {
            config.set('mdk_analytics_enabled',"true",function(err) {
                console.log('MDK CLI is collectngs data about its use and is sending them to our team.')
                console.log('if you don\'t want this information to be collected, change the mdk_analytics_enabled to false')
                return printFullNotice();
            });
        }
        else {
