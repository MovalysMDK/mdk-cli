...
function mdkThis() {
    ...
    program
        ...
        .action(function(mdkVersion){
            devtools.specs.get(mdkVersion, false, function(err, devtoolVersionObject) {
                if ( err ) {
    ...
    program
        ...
        .action(function(platform, mdkVersion){
            devtoolsCheck.check(platform, mdkVersion, function(err) {
                if ( err ) {
    ...
    program
        ...
        .action(function(platformName){
            platform.add(platformName, function(err) {
                if ( err ) {
                    exitWithMessage('Add platform ' + platformName + ' failed: ' + err, 1 );
                }
                else {
                    exitWithMessage('Platform ' + platformName + ' successfully added.', 0 );
                }
            });
        }).on('--help', function() {
        });
    ...
    program
        ...
        .action(function(platformName){
            ...
            platform.build(platformName, function(err) {
                if ( err ) {
                    exitWithMessage('Build platform ' + platformName + ' failed: ' + err, 1 );
                }
                else {
                    exitWithMessage('Platform ' + platformName + ' + successfully built.', 0 );
                }
            });
        }).on('--help', function() {
        });
