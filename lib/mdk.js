...
function mdkThis() {
    ...
    program
        .command('templates')
        .description('List aavaliable templates' + LINE_BREAK)
        ...
        .action(function (options) {
            ...
            templates.list(options.forceUpdate, function (err, result) {
                if (err) {
                    ...
                    templates.display(result, options.all, process.exit);
                }
            });
        }).on('--help', function () {
        });
...
// DEVTOOLS CHECKS -------------------------------------------------------
    ...
    program
        ...
        .action(function (platform, mdkVersion) {
            devtoolsCheck.check(platform, mdkVersion, false, function (err, noticeCallback) {
                if (err) {
                    exitWithMessage('Check failed: ' + err, 1, noticeCallback);
                }
                else {
