...
function mdkThis() {
    ...
    program
        ...
        .action(function( err, list ){
            ...
            versions.list( function(err, allVersions) {
                if ( err ) {
                    exitWithMessage('Failure reading version list: ' + err, 1 );
                }
                else {
                    for( var vers in allVersions ) {
    ...
    program
        ...
        .action(function(){
            if ( err ) {
                ...
                exitWithMessage('Version list.', 0 );
            }
        }).on('--help', function() {
        });
    ...
    program
        ...
        .action(function(mdkVersion){
            versions.get(mdkVersion, function(versionInfo, err) {
                if ( err ) {
                    exitWithMessage('Fail to find devtool version for mdk ' + mdkVersion + ': ' + err, 1 );
                }
                else {
                    console.log(versionInfo.devToolsVersion, 0 );
                    process.exit(0);
                }
            });
        }).on('--help', function() {
    ...
    program
        ...
        .action(function(platform, mdkVersion){
            devtools.install(platform, mdkVersion, function(err) {
                if ( err ) {
