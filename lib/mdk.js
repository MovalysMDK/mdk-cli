    ...
    config.get('mdk_analytics_enabled',function(err, res) {
        ...
        if (res === undefined)
        {
            config.set('mdk_analytics_enabled',"true",function(err) {
                var promptText = 'MDK CLI collects data about the way you use it.\n';
                promptText += 'We use this information to improve the MDK\n';
                promptText += 'You can disable this behavior by running the following command : "mdk config-set mdk_analytics_enabled false"\n';
                promptText = promptText + "press enter to continue.\n";
