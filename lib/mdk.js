    ...
    config.get('mdk_analytics_enabled',function(err, res) {
        ...
        if (res === undefined)
        {
            config.set('mdk_analytics_enabled',"true",function(err) {
                console.log('MDK CLI collects data about the way you use it.')
                console.log('You can disable this behavior by running the following command : "mdk config-set mdk_analytics_enabled false')
                return printFullNotice();
            });
        }
        else {
