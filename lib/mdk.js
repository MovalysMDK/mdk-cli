...
// Variables
var LINE_BREAK = '\n';
var AUTHORIZED_PLATFORM_LIST = 'android, ios, html5';
...
// DEVTOOLS CHECKS -------------------------------------------------------
    ...
    program
        .command('platform-deploy <platform> <appPath>')
        .description('Deploy the given platform of the current MDK project')
        .action(function (platformName, path, options) {
            options.deployDir = path;
...
function mdkQuickNotice () {
    ...
    console.log("  3. Create a new MDK project with " + clc.bold("mdk create <applicationId> -p <platforms>") + " (where platforms could be 'android', 'ios', 'html5' or a comma separated list of platforms ");
    console.log("     "+clc.italic("Example : mdk create com.soprasteria.myapp -p android,ios,html5"));
