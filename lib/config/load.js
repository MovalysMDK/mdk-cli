...
function load(readonly, callback) {
    ...
    var userConfDirectory = path.join(system.userHome(), '.mdk');
...
function initUserDirectory(userDirectoryPath, userFilePath,  callback) {
    ...
    //Check parameters
    assert.equal(typeof userDirectoryPath, 'string');
    assert.equal(typeof userFilePath, 'string');
    assert.equal(typeof callback, 'function');

    //Initializes user directory if doesn't exists
    fs.ensureDir(userDirectoryPath, function (err) {
        if (err) {
            callback(err);
        }
        else {
