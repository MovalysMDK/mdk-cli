...
function deploy( projectConf, toolSpecs, osName, callback ) {
    if (!projectConf.project.buildDisable && projectConf.deployDir) {
        ...
        var command = 'msbuild /p:AppxBundlePlatforms="Any CPU"/p:AppxBundle=Always/p:Configuration=DEBUG /p:OutputPath="' + projectConf.deployDir + '" ' ;
        runAsync(command + phoneAppDir, 'error: MSBuild failed on solution ' + phoneAppDir, function(err) {
            if (err) {
                callback(err);
            }
            else {
                fs.copySync(phoneAppx,projectConf.deployDir);
                mdkLog.ok('deploy : ' + phoneAppDir, 'success');
                command = 'msbuild /p:Platform=x86 /p:AppxBundle=Always /p:Configuration=DEBUG /p:OutputPath="' + projectConf.deployDir + '" ' ;
                runAsync(command + storeAppDir, 'error: MSBuild failed on solution ' + storeAppDir , function(err) {
                    if (err) {
                        callback(err);
                    }
                    else {
                        fs.copySync(storeAppx,projectConf.deployDir);
