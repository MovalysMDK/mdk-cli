    ...
    async.waterfall( [
        ...
        function( projectConf, cb) {
            config.get("dev_mode",function(err,result) {
                ...
                analytics.mdklog({"command":cmd,platform:platformName,
                            "version":projectConf.project.mdkVersion,
                            "project":projectConf.project.applicationId,
                            "dev_mode":dev_mode}, function(err) {
                    cb();
                });             
            });
        }
...
function parseCommandOptions(options, projectConf) {
    ...
    if( options.deployDir) {
        projectConf.deployDir = options.deployDir;
    }
    
    if(typeof options.verbose != "undefined" && options.verbose === true) {
        projectConf.options.mavenOptions.push('-X');
        projectConf.options.gradleOptions.push('-S');
		projectConf.options.bowerOptions.push('-V');
        projectConf.options.isVerbose = true;
    }
    else {
        projectConf.options.bowerOptions.push('--quiet');
        projectConf.options.gulpOptions.push('--silent');
    }
}
