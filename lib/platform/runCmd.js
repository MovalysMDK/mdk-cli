    ...
    async.waterfall( [
            ...
            systemConfig.get("tools_"+devToolsSpecs.version+"_"+platformName, function(err, toolsInstalled) {
                if (err) {
                    ...
                    if (toolsInstalled !== 'true') { // Tools aren't installed
                        cb("You need to install the tools for the "+platformName+" platform first.\r\n"+clc.underline("Use:")+clc.bold(" mdk tools-install " +platformName+ " " + projectConf.project.mdkVersion, projectConf));
                    } else {
        ...
        function(projectConf, devToolsSpecs, osName, cb) {
            ...
            if ( platformScript ) {
                ...
                cb('Unknown platform: ' + platformName, projectConf);
            }
        }
    ], function(err, projectConf) {
        if(err) {
            if(typeof  projectConf != 'undefined') {
                err  = {"message":err, "projectConf":projectConf};
            }
        }
        callback(err);
    });
