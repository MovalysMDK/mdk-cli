...
function deploy( projectConf, toolSpecs, osName, callback ) {
    if (!projectConf.project.buildDisable) {
        ...
        buildIpa(projectConf,
                ...
                curDir + '/ios/build',
                path.join(process.cwd(),'iosDeploy', projectConf.project.mdkVersion, projectConf.project.artifactId),
                'https://nansrvintc2.ntes.fr.sopra/apps/ios-native/' + projectConf.project.mdkVersion,  function (err) {
                    if (!err) {
