...
var renameIfExists = require('../../utils/io/renameIfExists');
...
function createIOSProjectFromTemplate( projectConf, toolSpecs, osName, callback ) {
    ...
    async.waterfall([
        function(cb) {
            renameIfExists('pom.xml', 'pom.xml.backup');
            cb();
        },
                ...
                if (err) {
                    ...
                    async.filter(
						...
						function(platform, itemCb) { 
							itemCb( platform.name === "ios" );
						},
						function(results){
							if ( results.length === 0 ) {
								return cb("Platform ios is not available for this template (or not in this version)");
        ...
        function(cb) {
            ...
            renameIfExists('pom.xml.backup', 'pom.xml');
            if (err) {
                callback(err);
            }
            else {
