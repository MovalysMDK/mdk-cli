...
function deploy( projectConf, toolSpecs, osName, callback ) {
    if (!projectConf.project.buildDisable) {
        ...
        deployApk( apkFile, path.join(process.cwd(),'androidDeploy', projectConf.project.mdkVersion), projectConf, function (err) {
            if (!err) {
                mdkLog.ok('deploy on androidDeploy' , 'success');
            }
            callback();
        });
    } else {
