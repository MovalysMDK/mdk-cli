...
module.exports = {
    ...
    install: function( toolSpec, osName, callback) {
        ...
        async.waterfall([
            function(cb) {
                fs.createReadStream(archiveFile).pipe(unzip.Extract({ path: toolSpec.opts.workDir })
                        .on('error', function(zipErr) {
                            cb(zipErr);
                        })
                        .on('close', function() {
                            console.log("  Zip archive extracted with success");
                            console.log("  Move apache-maven to tools directory");
                            fs.move(path.join (toolSpec.opts.workDir, toolSpec.name + "-" + toolSpec.version), toolSpec.opts.installDir, {clobber:true}, function(moveErr) {
                                cb(moveErr);
                            });
                        })
                );
            ...
            function(cb) {
                console.log("  Restore permissions");
                fs.chmod(path.join(installDir, "bin", "mvn"), '700', cb);
            },
            function(cb) {
                fs.chmod(path.join(installDir, "bin", "mvnDebug"), '700', cb);
            },
            function(cb) {
                fs.chmod(path.join(installDir, "bin", "mvnyjp"), '700', cb);
            },
            function(cb) {
