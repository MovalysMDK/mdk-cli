...
module.exports = {
    ...
    install: function( toolSpec, callback) {
        ...
        var extractedTarPath = path.join(toolSpec.opts.workDir,extratedTarFilename);
        ...
        async.waterfall([
            function(cb) {
                ...
                //'cat', ['Payload', '|', 'gunzip', '-dc', '|', 'cpio', '-i']
                console.log("  extract JDK compressed tar file");

                var gunzip = zlib.createGunzip();
                var rstream = fs.createReadStream(cacheFile);
                var wstream = fs.createWriteStream(extractedTarPath);
                ...
                wstream.on('finish', function(){
                    console.log("  remove JDK compressed tar file");
                    cb();
                });
            },
            function(cb) {
                    var extract = tar.extract(path.join(toolSpec.opts.workDir, "JDK"));
                    extract.on('finish', cb);
                    extract.on('error', cb);
                    fs.createReadStream(extractedTarPath).pipe(extract);
            ...
            function(cb) {
                console.log("  remove file: " + extractedTarPath);
                fs.remove(extractedTarPath, cb);
            },
            function(cb) {
