...
module.exports = {
    ...
    install: function( toolSpec, osName, callback) {
        ...
        var installAndroidSdk = require("./installAndroidSdk.win");
    ...
    extractZip: function(zipFile, toolSpec, callback) {
        ...
        checkAndroidSdkFolder( toolSpec.opts.installDir, toolSpec.osName, function(err) {
            if (err) {
                process.chdir(toolSpec.opts.devToolsBaseDir);
                console.log("  extract zip: " + zipFile);
                fs.createReadStream(zipFile).pipe(unzip.Extract({ path: toolSpec.opts.installDir })
                        .on('error', function(zipErr) {
                            callback(zipErr);
                        })
                        .on('close', function() {
                            console.log("  Zip archive extracted with success");
                            console.log("  Move android-sdk to tools directory");
                            process.chdir(toolSpec.opts.installDir);
                            fs.move("android-sdk-windows", toolSpec.opts.installDir, function(moveErr) {
                                callback(moveErr);
                            });
                        })
                );
    ...
    updateSdk: function(toolSpec, callback) {
        ...
        installAndroidSdk.createInstallScript(toolSpec, function(err, installScript) {
            ...
            if ( err) {
                ...
                console.log(clc.red.bold("INSTALL SCRIPT = ", installScript));
    ...
    createInstallScript: function(toolSpec, callback) {
        ...
        var installScriptTemplateFile = path.join( __dirname, "installSdk.template.bat");
        var installScriptFile = path.join( toolSpec.opts.workDir, "installSdk.bat");
