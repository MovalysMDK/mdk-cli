...
module.exports = {
    ...
    updateSdk: function(toolSpec, osName, callback) {
        ...
        async.waterfall([
                ...
                function(proxyInfo, cb) {
                    ...
                    console.log("  create install script.");

                    installAndroidSdk.createInstallScript(toolSpec, osName, proxyInfo, function(err, installScript) {
                        if ( err) {
                            cb(err);
                        }
                        else {
                            cb(null, installScript);
                        }
                    });
                },
                function(installScript, cb) {

                    console.log("  run install script.");

                    var cmd = spawn(installScript, [], { encoding: 'utf8', stdio: ['pipe', process.stdout, process.stderr] } );
                    ...
                    cmd.on('close', function (code) {
                        ...
                        if (code !== 0) {
                            cb('Android SDK Update failed: '+ installScript );
                        }
                        else {
                            cb();
                        }
     ...
     * @param callback callback
     */
    createInstallScript: function(toolSpec, osName, proxyInfo, callback) {
        ...
        var installScriptTemplateFile;
        var installScriptFile;
        switch(osName) {
            case "osx":
                installScriptTemplateFile = path.join( __dirname, "installSdk.template.sh");
                installScriptFile = path.join( toolSpec.opts.workDir, "installSdk.sh");
                break;
            case "win":
                installScriptTemplateFile = path.join( __dirname, "installSdk.template.bat");
                installScriptFile = path.join( toolSpec.opts.workDir, "installSdk.bat");
                break;
        }
        ...
        if ( typeof proxyInfo.proxyHostname !== "undefined" ) {
            proxyParams += " --proxy-host " + proxyInfo.proxyHostname ;
        }
        if ( typeof proxyInfo.proxyPort !== "undefined" ) {
            proxyParams += " --proxy-port " + proxyInfo.proxyPort ;
        }

        var androidCmd = path.join(toolSpec.opts.installDir, "tools", "android");
        var androidCmdInstallMain = androidCmd + " update sdk -u --filter "+ filter + proxyParams;
            ...
            function (isInstalled, componentList, cb) {
                ...
                if ( isInstalled === false ) {
                    ...
                    if (buildToolId === -1) {
                        ...
                        objects.cmdBuildTools = androidCmd + " update sdk -u --all --filter " + buildToolId + proxyParams;
                        cb();
                    }
                }
                else {
