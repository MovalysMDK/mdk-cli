...
function loadVersion(devToolsVersion, callback ) {
    ...
    var test = '{ "mdk-cli": { "minVersion": "1.0.0" }, "install": [' +
        '{  "name": "jdk", "version": "1.7.0.45", "script": "jdk/installJdk", "opts": {}, "diskSpace": 30 }' +
        ']}';

    //return json in callback
    callback(null, JSON.parse(test));
}
...
function retrieveDevToolSpec( devToolsVersion, forceUpdate, callback ) {
    ...
    fs.ensureDir(cacheDevToolDir, function(err) {
        if ( err) {
            callback(err);
        }
        else {
