...
var checkXcprojFolder = require('./checkXcprojFolder');
var checkXcprojVersion = require('./checkXcprojVersion');
...
module.exports = {
    ...
    check: function( checkSpec, devToolsSpec, osName, platform, callback ) {
        ...
        async.waterfall( [
            function (cb) {
                // find tool spec for xcproj
                devToolsSpecs.findToolSpec(devToolsSpec, "xcproj", osName, platform, cb);
            },
            function (results, cb) {
            ...
            function ( toolSpec, installDir, cb) {
                // check directory of xcproj is ok.
            ...
            function ( toolSpec, installDir, cb) {
                ...
                checkXcprojVersion( toolSpec.version, installDir, function(err) {
                    if (err) {
                        cb(err, toolSpec, installDir);
                    }
                    else {
            ...
            if ( err ) {
                ...
                console.log(clc.green('[OK]') + " xcproj version: " + toolSpec.version );
                callback(true);
            }
        });
    }
