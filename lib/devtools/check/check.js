...
function checkPlatform( platform, osName, callback) {
    ...
    switch(platform) {
        case 'ios':
            system.isWin(function (isWin) {isWin ? callback('iOS is not supported on Windows platform') : callback()});
            system.isLinux(function (isLinux) {isLinux ? callback('iOS is not supported on Linux platform') : callback()});
            break;
        case 'android':
            //Supported everwhere
            callback();
