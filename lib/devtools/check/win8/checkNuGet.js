...
var checkNuGetVersion = require('./nuget/checkNuGetVersion');
...
module.exports = {
    ...
    check: function( checkSpec, devToolsSpec, osName, platform, callback ) {
        ...
        async.waterfall( [
            function (cb) {
                // find tool spec for NuGet
                devToolsSpecs.findToolSpec(devToolsSpec, "NuGet", osName, platform, cb);
            },
            function (results, cb) {
                var toolSpec = results[0];
                // read configuration to know where NuGet is installed.
            ...
            function ( toolSpec, installDir, cb) {
                // check NuGet version
                checkNuGetVersion( toolSpec.version, installDir, function(err) {
                    if (err) {
