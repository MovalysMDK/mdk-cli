"use strict";

var spawn = require('child_process').spawn;
var fs = require('fs');
var clc = require('cli-color');

var mdkLog = require('../log');

module.exports = function(script, callback){

	if ( ! /^darwin|^win/.test(process.platform) ){
		return callback("Only Mac and Windows supports MDK Shell. Please use platform-env command");
	}

    script = script
                .replace(/(?:\r?\n\s*\r?\n|\r?\n)/g, ' && ')
                .replace(/(^(\s*&&)*)/g, '')
                .replace(/((\s*&&\s*)*$)/g, '');

    var command = "cmd.exe";
	var parameters = ['/c', 'start', 'cmd', '/T:03', '/K', script];
    
	if ( /^darwin/.test(process.platform) ){
		// Additional escaping for Mac
		script = script
                    .replace(/\\/g, '\\\\')
                    .replace(/"/g, '\\"');
        command = "osascript";
        parameters = ['-e', 'tell application "Terminal" to activate', '-e', 'tell application "Terminal" to do script "' + script + '"' ];
	}

	var shell = spawn( command, parameters );
	
	shell.on('error', function(err){
		callback("An error occured while running MDK shell : " + err);
	});
	
	shell.on('close', function(code){
		if (code === 0){
			mdkLog.ok("MDK Shell", "Successfully spawned");
			return callback();
		}else{
			return callback("MDK Shell exited with error code " + code);
		}
	});
}
