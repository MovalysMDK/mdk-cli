...
function buildIpa( conf, appDir, ipaDir, deployDir, baseUrl, callbackGen ) {
    ...
    var xcrunCmd = '/usr/bin/xcrun -sdk iphoneos PackageApplication -v ' + appDir + ' -o ' + deployDir + '/' + conf.project.artifactId + '/' + ipaName ;
    
    fs.mkdirs(deployDir + '/' + conf.project.artifactId, function (err) {
        runAsync(xcrunCmd, 'error: xcrun failed', function(err, result) {
            if ( err ) {
                callbackGen(err);
            }
            else {
                callbackGen(err,result);
            }
        });
    });
}
