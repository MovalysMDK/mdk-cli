...
function deployWin8( AppDir, deployDir, buildTarget, archi, conf, buildNr, callback ) {
    ...
    mkdirSync(path.join(deployDir,minor,conf.project.artifactId,buildNr));
    ...
    runAsync(command + AppDir, 'error: MSBuild failed on solution ' + AppDir , function(err) {
        var output = fs.createWriteStream(path.join(deployDir, minor, conf.project.artifactId, buildNr, conf.project.artifactId + buildTarget + '-' + buildNr + '.zip'));
        archive.pipe(output);
        archive.bulk([
