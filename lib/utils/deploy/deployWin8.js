...
function deployWin8( AppDir, deployDir, buildTarget, archi, conf, buildNr, callback ) {
    ...
    runAsync(command + AppDir, 'error: MSBuild failed on solution ' + AppDir , function(err) {
        ...
        fs.copySync( path.join(deployDir, minor, conf.project.artifactId, conf.project.artifactId + '-' + buildNr + '-' + buildTarget + '.zip') ,
                     path.join(deployDir, minor, conf.project.artifactId, conf.project.artifactId + '-' + buildTarget + '.zip'), {forceDelete: true});
