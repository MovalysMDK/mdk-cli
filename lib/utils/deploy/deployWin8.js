...
function deployWin8( AppDir, deployDir, buildTarget, archi, conf, buildNr, callback ) {
    ...
    mkdirSync(deployDir);
    mkdirSync(path.join(deployDir,conf.project.mdkVersion));
    mkdirSync(path.join(deployDir,conf.project.mdkVersion,conf.project.artifactId));
    mkdirSync(path.join(deployDir,conf.project.mdkVersion,conf.project.artifactId,buildNr));
    ...
    runAsync(command + AppDir, 'error: MSBuild failed on solution ' + AppDir , function(err) {
        var output = fs.createWriteStream(path.join(deployDir, conf.project.mdkVersion, conf.project.artifactId, buildNr, conf.project.artifactId + buildTarget + '-' + buildNr + '.zip'));
        archive.pipe(output);
        archive.bulk([
