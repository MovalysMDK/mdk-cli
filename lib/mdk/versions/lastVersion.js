    ...
    async.waterfall( [
        ...
        function(snapshotEnable, cb) {
            ...
            load(false, function(err, versions) {
                if (err) {
                    ...
                    versions.versions.forEach(function (item) {
                            if (item.version.indexOf("-SNAPSHOT") != -1 && snapshotEnable) {
                                return;
                            }
			...
			*/
			...
			list(function(err, versionObject){
				if (err){
					return cb(err);
				}
                var lastVersion = "0.0.0-FAKE";
                var i = versionObject.versions.length;
                while ( i-- ) {
                    // Search the SNAPSHOT or greater version
                    if (semver.gt(versionObject.versions[i].version, lastVersion)) {
                        lastVersion = versionObject.versions[i].version;
                    }
                    // If default found, break everything and return this one
                    if ( versionObject.versions[i].hasOwnProperty("default") && versionObject.versions[i].default){
                        break;
                    }
                }
                
				if (lastVersion !== '0.0.0-fake') {
					cb(null, lastVersion);
				} else {
					cb("No version found");
				}
