...
function retrieveLastVersions( forceUpdate, callback ) {
    ...
    var cacheMdkVersionsFile = path.join( cacheDir, "mdkversions.json");
    var options = {};
            ...
            if ( doDownload ) {
                ...
                config.get('mdkUrl', function (err, baseUrl) {
                    if (err) {
                        ...
                        network.downloadFile(options, cacheMdkVersionsFile, function (err) {
                            if (err) {
                                cb(err);
                            }
                            else {
...
function loadVersions( callback ) {
    ...
    //Load versions
    var cacheMdkVersionsFile = path.join(system.userHome(), '.mdk', 'cache', 'mdkversions.json');
