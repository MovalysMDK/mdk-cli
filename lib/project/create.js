...
function create( applicationId, options, callback ) {
    ...
    if (! /^([a-z]+\.)+[a-z0-9]+$/.test(applicationId) ) {
        callback("applicationId must be lowercase Alphabet only and must contain at least one dot.\n"+
                 "        Note that the last part can contain digits.\n"+
    ...
    async.waterfall([
            ...
            function (templateVersionObject, templateSpec, cb) {
                ...
                if(typeof templateVersionObject.zipFile === 'undefined') {
                    filename = 'mdk-project-' + conf.project.version + '-bin.zip';
        ...
        function (username, password, cb) {
            config.getList(["mdkRepoRelease","mdkRepoSnapshots", "snapshotEnable"], function (err, values) {
                if (err) {
                    ...
                    if ( values.snapshotEnable === "true" && isSnapshot(conf.template.version)) {
