...
function create( applicationId, mdkVersion, callback ) {
    ...
    if ( applicationId.indexOf(".") == -1 ) {
        callback("applicationId must be contains at least one dot. Example: com.mycompany.projectId.");
        return;
    }
...
function downloadZip( conf, callback ) {
    ...
    var cacheDir = path.join( system.userHome(), '.mdk', 'cache', 'zip' );
    ...
    async.waterfall([
        ...
        function (username, password, cb ) {
            ...
            var options = {
                url: 'https://artifactory.movalys.sopra.com/artifactory/prj-mdk-releases/com/sopragroup/adeuza/movalysfwk/mdk-project/'
                    + conf.project.mdkVersion +'/' + filename
            };
