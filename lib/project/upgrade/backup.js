'use strict';

/**
 * Imports
 */
var fse = require('fs-extra');
var path = require('path');

var mdkLog = require('../../utils/log');
var mdkPath = require('../../mdkPath')();

function backup( callback ){

	// Initialize 
	
	
	// Running a "." printing loop to notify the user that something is happening
	// as we can't access the progress of the copy function
	var waitingInterval = setInterval(function(){
		process.stdout.write('.');
	}, 200);

	// Copy parameters
	var currentFormattedDate = new Date().toISOString().replace(/T/, '_').replace(/\..+/, '').replace(/:/g, '-');
	var backupFolder = path.join(mdkPath.tmpDir, path.basename( process.cwd() ) + "__BAK_" + currentFormattedDate);
	var copyOptions = {
		clobber: true,
		preserveTimestamps: true
	};

	mdkLog.notice("Backup", "Backing up project. Please Wait " + backupFolder);
	// Copy folder to .mdk/temp/<CURRENT_PROJECT_FOLDER>__BAK_<YYY-MM-DD_HH-mm-SS>
	fse.copy( './', backupFolder, copyOptions, function(err){

		// Stopping the "." print loop
		clearInterval( waitingInterval );
		process.stdout.write('\n\r');

		if (err){
			return callback(err);
		}

		return callback(null, backupFolder);

	});
}

module.exports = backup;