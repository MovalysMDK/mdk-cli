...
function upgradeProject( templateTarget, mdkTarget, callback ){
	...
	async.waterfall([
		...
		function(cb){
			fse.exists( path.join( process.cwd(), 'mdk-project.json'), function(exists){
				if ( ! exists ){
					return cb("The mdk-project.json file could not be found");
				}else{
					return cb();
				}
			})
		...
		function(cb){
			fse.readJSON( path.join( process.cwd(), 'mdk-project.json'), function(err, obj){
				if (err){
					return cb(err);
				}
		...
		// Save file
		function(json, cb){
			fse.writeJSON( path.join(process.cwd(), "mdk-project.json"), json, function(err){
				if (err){
					return cb(err);
				}
