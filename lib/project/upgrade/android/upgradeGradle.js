	...
	async.waterfall([
		...
		function(gradle, cb){
            ...
            mdkVersions.get( mdkTarget, false, function(err, v){
                ...
                gradle = gradle.replace(/(.*?mdkAndroidVersion *?= *?["'])([^"']*)(["'].*)/mi, "$1" + v.android.mdkAndroidVersion + "$3");
                gradle = gradle.replace(/(.*?mdkWidgetVersion *?= *?["'])([^"']*)(["'].*)/mi, "$1" + v.android.mdkWidgetVersion + "$3");
                gradle = gradle.replace(/(.*?mdkEspressoVersion *?= *?["'])([^"']*)(["'].*)/mi, "$1" + v.android.mdkEspressoVersion + "$3");
