...
function upgrade(target, callback){
	...
	async.waterfall([
		...
		function(templateName, currentVersion, templateTarget, mdkTarget, cb){
			...
			// Check upgrade from:to snapshot
			if ( currentVersion.indexOf('SNAPSHOT') || templateTarget.indexOf('SNAPSHOT') ){
				return cb("Automatic upgrade is not available from / to a SNAPSHOT version");
			}
